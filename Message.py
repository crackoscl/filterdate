    ...
    if tipo_dato == 70:
        for dato in data:
            f_actual = datetime.strptime(
                dato['created_at'], "%Y-%m-%d %H:%M:%S") - timedelta(hours=timezone)
            if f_actual >= f_desde and f_actual <= f_hasta and dato['equipos_id'] == id_bus:
                datos.append(dato)
        ...
        for dato in data:
            f_actual = datetime.strptime(
                dato['created_at'], "%Y-%m-%d %H:%M:%S") - timedelta(hours=timezone)
            if f_desde <= f_actual and f_actual <= f_hasta and dato['id_bus'] == id_bus:
                dato.append(dato)

    return datos


data_list = [{'id': 224495167, 'valor': '85387.42', 'tipos_datos_id': 70, 'created_at': '2022-01-04 04:10:58', 'equipos_id': 85, 'nombre': 'BUS 32'}, {'id': 224494680, 'tipos_datos_id': 70,
